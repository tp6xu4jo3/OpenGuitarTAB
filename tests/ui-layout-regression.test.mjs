...
assert.match(structure,/function makeRowMoreButton\(rowIndex, visualRowIndex\)/);
assert.match(structure,/makeVisualRowLabel\(rowIndex, visualRowIndex\)/);
assert.match(structure,/nodes\.forEach\(node => node\.classList\.add\('is-selected'\)\)/,'selecting a logical row highlights all wrapped visual rows');
...
assert.match(responsiveCss,/@media \(max-width:760px\)[\s\S]*\.play-panel\{display:grid;grid-template-columns:auto auto minmax\(76px,1fr\)/,'mobile playback controls should use a compact two-row grid');
assert.match(responsiveCss,/\.editor-ribbon-section\{[^}]*overflow-x:auto[^}]*flex-wrap:nowrap/s,'mobile ribbon should scroll horizontally instead of overflowing the viewport');
