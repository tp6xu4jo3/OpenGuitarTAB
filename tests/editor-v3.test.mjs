...

{
  ...
  const beforeMove = buildSystems(structure);
  const movedMeasureId = beforeMove[0][0].id;
  const exchangedMeasureId = beforeMove[1][0].id;
  structure = moveMeasureAt(structure, 0, 0, 1, 1, { idFactory: ids }).document;
  assert.equal(buildSystems(structure)[0].at(-1).id, exchangedMeasureId, 'downward cross-row move must return target first measure to source end');
