...
function installSongNormalizer() {
...
function installRhythmRenderer() {
  const rhythmPositionPercentForRow = (position, rowIndex) => ((position + 1) / rowPositionCount(rowIndex)) * 100;
...
function installFocusNavigation() {
...
export function installGridLayoutAdapter() {
  if (installed || typeof window === 'undefined') return;
  installed = true;
  installRowMetrics();
  installSongNormalizer();
  installGridFactory();
  installRhythmRenderer();
