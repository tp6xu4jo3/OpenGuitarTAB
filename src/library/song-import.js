...
export async function importSongFile(file) {
  ...
  try {
    const raw = typeof window.deserializeSong === 'function'
      ? window.deserializeSong(await file.text())
      : JSON.parse(await file.text());
    const imported = typeof window.normalizeSongRecord === 'function'
      ? window.normalizeSongRecord(raw)
      : raw;
    ...
    if (!imported.id || records.some(song => song.id === imported.id)) {
      imported.id = typeof window.uid === 'function' ? window.uid() : `song-${Date.now().toString(36)}`;
    }
    if (!imported.name) imported.name = file.name.replace(/\.json$/i, '') || '匯入曲譜';
    imported.createdAt = Number(imported.createdAt) || Date.now();
    imported.updatedAt = Date.now();

    const saved = await window.persistSongToCloud(imported);
